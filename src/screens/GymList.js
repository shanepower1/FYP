...
function GymList() {
  ...
  useEffect(() => {
    if (search.length < 1) {
      //If there is nothing entered into the search bar no list of registered gyms will appear.
      setFilteredGyms([]);
    } else {
      let tempSearch = search.toLowerCase(); //tempSearch is what the user will be entering

      // Loops through gyms, if the gym doesn't pass any of the conditions it is filtered out. 
      ...
      ); //if the county entered into the search bar matches that off a gym in the db it will appear
    ...
    <>
      {
        <MyView background="#2F0B29">
          <Image
            source={require("assets/gymconnect.png")}
            height={200}
            style={{
              marginTop: 25,
              marginBottom: 25,
          ...
          <Input
            ...
            value={search}
            placeholder="First lets search for your gym"
          ...
          <View>
            {
              ...
              filteredGyms.map((item) => (
                <TouchableOpacity
                  key={item.id}
                  onPress={() => joinAlert(item.id, item.name)}
                >
                  <ListItem
                    bottomDivider
                    ...
                    <ListItem.Content>
                      ...
                      <ListItem.Subtitle>
                        <Text style={{ color: "white" }}>
                          {item.address1.trim()}
                          {item.address2 == "" ? "" : ","}
                          {item.address2}, {item.town}, {item.county}
                        </Text>
                      </ListItem.Subtitle>
                    </ListItem.Content>
