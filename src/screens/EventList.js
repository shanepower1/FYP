...
function EventList({ navigation, route }) {
  ...
  const { gymId, userType } = useAuth(); // Alows access to gymId and userType variables from AuthContext. 
  ...
  // Loads all events from database for this gym. 
  function loadEvents() {
    getEvents(gymId)
      .then((events) => {
        setEvents(events);
  ...
  //https://www.npmjs.com/package/react-native-floating-action. Code for buttons inside floating action button. 
  const fabActions = [
    {
      text: "Refresh",
        ...
        <Card containerStyle={{ padding: 0 }}>
          {
            ...
            events.map((event) => (
              ...
              >
                <Avatar
                  source={{
                    uri: `https://firebasestorage.googleapis.com/v0/b/shanefyp-e17f7.appspot.com/o/events%2F${event.id}?alt=media&token=cff1649c-2042-4225-90a3-bb655d6d8b2c`,
                  }}
                />
                <ListItem.Content>
                  <ListItem.Title>
